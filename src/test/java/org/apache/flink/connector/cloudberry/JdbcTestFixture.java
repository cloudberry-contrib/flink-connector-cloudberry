...
public class JdbcTestFixture {
  ...
  @SuppressWarnings("deprecation") // Using legacy API for compatibility with Flink test framework
  public static final RowType ROW_TYPE =
      (RowType) fromLegacyInfoToDataType(ROW_TYPE_INFO).getLogicalType();

  // DataType array for field types (used by JdbcOutputFormatBuilder)
  // Schema: id (INT), title (STRING), author (STRING), price (DOUBLE), qty (INT)
  @SuppressWarnings("deprecation") // Using legacy API for compatibility with Flink test framework
  public static final DataType[] FIELD_DATA_TYPES =
      new DataType[] {
        fromLegacyInfoToDataType(BasicTypeInfo.INT_TYPE_INFO),
        fromLegacyInfoToDataType(BasicTypeInfo.STRING_TYPE_INFO),
        fromLegacyInfoToDataType(BasicTypeInfo.STRING_TYPE_INFO),
        fromLegacyInfoToDataType(BasicTypeInfo.DOUBLE_TYPE_INFO),
        fromLegacyInfoToDataType(BasicTypeInfo.INT_TYPE_INFO)
      };

  // TypeInformation for RowData (used by JdbcOutputFormatBuilder)
  public static final TypeInformation<RowData> ROW_DATA_TYPE_INFO = InternalTypeInfo.of(ROW_TYPE);
  ...
  private static void createWordsTable(Connection conn) throws SQLException {
    executeUpdate(
        conn,
        "create table " + WORDS_TABLE + " (id int not null, word varchar(50), primary key (id))");
  }
