...
import org.apache.flink.connector.cloudberry.common.JdbcConnectionProvider;
...
public class JdbcOutputFormat<In, JdbcIn, JdbcExec extends JdbcBatchStatementExecutor<JdbcIn>>
  ...
  public JdbcOutputFormat(
      ...
      @Nonnull JdbcWriteOptions writeOptions,
  ...
  public void open(int taskNumber, int numTasks) throws IOException {
    ...
    if (writeOptions.getBatchIntervalMs() != 0 && writeOptions.getBatchSize() != 1) {
      this.scheduler =
  ...
  public final synchronized void writeRecord(In record) throws IOException {
    ...
    try {
      ...
      batchCount++;
      if (writeOptions.getBatchSize() > 0 && batchCount >= writeOptions.getBatchSize()) {
        flush();
      }
    } catch (Exception e) {
  ...
  protected void addToBatch(In original, JdbcIn extracted) throws Exception {
  ...
  public synchronized void flush() throws IOException {
    ...
    for (int i = 0; i <= writeOptions.getMaxRetries(); i++) {
      try {
        attemptFlush();
        batchCount = 0;
        break;
        ...
        if (i >= writeOptions.getMaxRetries()) {
  ...
  protected void attemptFlush() throws Exception {
  ...
  public void updateExecutor(boolean reconnect) throws Exception {
    jdbcStatementExecutor.closeStatements();
